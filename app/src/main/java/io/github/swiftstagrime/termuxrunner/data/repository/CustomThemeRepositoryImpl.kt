...
class CustomThemeRepositoryImpl @Inject constructor(
    ...
    override fun getAllCustomThemes(): Flow<List<CustomTheme>> =
        themeDao.getAllThemes().map { list -> list.map { it.toDomain() } }

    override suspend fun getThemeById(id: Int): CustomTheme? =
        themeDao.getThemeById(id)?.toDomain()

    override suspend fun saveTheme(theme: CustomTheme): Long {
        return themeDao.insertTheme(theme.toEntity())
    }
