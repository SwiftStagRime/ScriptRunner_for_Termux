...
object AutomationFormatter {
    fun formatNextRun(context: Context, nextRun: Long?): String {
        ...
        return if (duration <= 0) {
            ...
            val relative = DateUtils.getRelativeTimeSpanString(
                nextRun, now, DateUtils.MINUTE_IN_MILLIS
            )
            context.getString(R.string.automation_next_run_format, relative)
        }
    }

    fun formatLastRun(context: Context, lastRun: Long?, exitCode: Int?): String {
        if (lastRun == null) return context.getString(R.string.automation_never_run)

        val relative = DateUtils.getRelativeTimeSpanString(
            lastRun, System.currentTimeMillis(), DateUtils.MINUTE_IN_MILLIS
        )
