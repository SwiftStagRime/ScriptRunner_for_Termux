...
class ScriptRepositoryImplTest {
    ...
    private fun validScriptJson(id: Int, name: String) = """
    ...
    private fun setupMockFile(fileName: String, content: String): Uri {
    ...
    @Test
    fun `importScripts links automations to new script IDs`() = runTest {
        val json = """
        {
            "categories": [],
    ...
    @Test
    fun `importSingleScript detects python and adds shebang`() = runTest {
    ...
    @Test
    fun `importScripts preserves all entity fields including presets`() = runTest {
        val json = """
        {
            "categories": [],
            "scripts": [{
    ...
    @Test
    fun `importScripts deduplicates categories by name`() = runTest {
        val json = """{
    ...
    @Test
    fun `importScripts handles legacy list format`() = runTest {
    ...
    @Test
    fun `importSingleScript does not add redundant shebang if already present`() = runTest {
    ...
    @Test
    fun `importSingleScript for compiled languages omits shebang`() = runTest {
    ...
    @Test
    fun `importScripts saves base64 icons to local storage`() = runTest {
    ...
    @Test
    fun `importScripts handles malformed json with failure result`() = runTest {
    ...
    @Test
    fun `updateScriptsOrder delegates correctly to dao`() = runTest {
    ...
    @Test
    fun `exportScripts includes all complex fields in json output`() = runTest {
    ...
    @Test
    fun `importScripts correctly maps all automation constraint fields`() = runTest {
        val json = """
        {
            "version": 3,
            "categories": [],
    ...
    @Test
    fun `importScripts handles legacy V2 format by providing missing required fields`() = runTest {
        val json = """
        {
            "version": 2,
            "categories": [],
    ...
    @Test
    fun `importScripts correctly maps all script fields to entity during import`() = runTest {
        val json = """
        {
            "categories": [],
            "scripts": [{
    ...
    @Test
    fun `importScripts correctly reconstructs category relationships`() = runTest {
        val json = """
        {
            "categories": [
        ...
        val categoryIdCounter = java.util.concurrent.atomic.AtomicInteger(10)
    ...
    @Test
    fun `importScripts handles Version 1 format (raw array) correctly`() = runTest {
        val json = """
        [
            {
                "name": "LegacyV1",
    ...
    @Test
    fun `importScripts handles Version 2 format (object with scripts only) correctly`() = runTest {
        val json = """
        {
            "scripts": [
                {
    ...
    @Test
    fun `importScripts fills default values for missing optional DTO fields`() = runTest {
        val json = """
        {
            "scripts": [
                {
    ...
    @Test
    fun `exportScripts produces JSON compatible with Version 3 specification`() = runTest {
        val script = ScriptEntity(
            name = "ExportTest",
            code = "pwd",
            interpreter = "sh",
    ...
    @Test
    fun `importScripts maintains data integrity when envVars are complex`() = runTest {
        val json = """
        {
            "scripts": [
                {
    ...
    @Test
    fun `importSingleScript correctly handles shebang with whitespaces and newlines`() = runTest {
        val rawContent = "   \n\n#!/bin/sh\necho 1"
